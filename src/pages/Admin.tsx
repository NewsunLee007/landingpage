...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = useState<'apps' | 'articles' | 'comments'>('apps');
  ...
  }, [isAuthenticated, setApps, setArticles, activeTab]);
  ...
  const handleApproveComment = async (commentId: string) => {
    try {
      await apiService.updateComment(commentId, { status: 'approved' });
      fetchComments();
    } catch (error) {
  ...
  const handleRejectComment = async (commentId: string) => {
    try {
      await apiService.updateComment(commentId, { status: 'rejected' });
      fetchComments();
    } catch (error) {
  ...
  const handleSaveApp = async () => {
    ...
    try {
      if (currentApp.id) {
        ...
        const newApp = {
          ...currentApp,
          id: currentApp.id || crypto.randomUUID(),
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-stone-800 font-sans dark:bg-[#0F0F0F] dark:text-stone-200">
        ...
        <div className="flex items-center gap-6">
          <div className="flex gap-2">
            ...
            <button 
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${activeTab === 'comments' ? 'bg-[#2A6049] text-white dark:bg-[#4A8069]' : 'bg-stone-50 text-stone-600 hover:bg-stone-100 dark:bg-stone-800 dark:text-stone-400 dark:hover:bg-stone-700'}`}
            >
              评论管理
            </button>
          </div>
      ...
      <main className="max-w-5xl mx-auto p-6 mt-6">
        ...
        {activeTab === 'comments' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold font-serif">评论管理</h2>
              <button 
                onClick={fetchComments}
                className="flex items-center gap-2 bg-stone-100 text-stone-700 px-4 py-2 rounded-xl hover:bg-stone-200 transition-colors text-sm font-medium dark:bg-stone-800 dark:text-stone-300 dark:hover:bg-stone-700"
              >
                刷新评论
              </button>
            </div>
            ...
            {isLoadingComments ? (
              ...
              <div className="space-y-4">
                {comments.map((comment) => (
                  <div key={comment.id} className="bg-white p-6 rounded-3xl shadow-sm border border-stone-100 dark:bg-[#1A1A1A] dark:border-stone-800">
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center gap-3">
                          <h4 className="font-medium text-stone-800 dark:text-stone-100">{comment.author}</h4>
                          {comment.email && (
                            <span className="text-xs text-stone-400 dark:text-stone-500">{comment.email}</span>
                          )}
                        </div>
                        <p className="text-sm text-stone-400 font-mono mt-1 dark:text-stone-500">{new Date(comment.createdAt).toLocaleString()}</p>
                        <p className="mt-4 text-stone-600 dark:text-stone-300">{comment.content}</p>
                      </div>
                      <div className="flex gap-2">
                        <button 
                          onClick={() => handleApproveComment(comment.id)}
                          className="p-2 text-green-500 hover:bg-green-50 rounded-xl transition-colors dark:hover:bg-green-900/20"
                          title="批准"
                        >
                        ...
                        <button 
                          ...
                          className="p-2 text-amber-500 hover:bg-amber-50 rounded-xl transition-colors dark:hover:bg-amber-900/20"
                          title="拒绝"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="text-center py-10 text-stone-400 dark:text-stone-500">暂无待审核评论</div>
            )}
          </div>
        )}
      </main>
